...
ssh -i ~/.ssh/travis_rsa $VPS_USERNAME@$VPS_HOST "cd /root/docker-compose && export TAG_NAME=latest && export BOT_TOKEN=${BOT_TOKEN} && export BOT_USERNAME=${BOT_USERNAME} && export PAYMENTS_STRIPE_KEY_PUBLIC=${PAYMENTS_STRIPE_KEY_PUBLIC} && export PAYMENTS_STRIPE_KEY_SECRET=${PAYMENTS_STRIPE_KEY_SECRET} && docker-compose pull && docker-compose up --detach --build"
